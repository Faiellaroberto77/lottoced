...
// ---------------------------------------------- PRENDE DATE -------------------------------------------
$g_sql = "select EsData from es_winforlife_grattacielo group by esdata order by esdata desc limit $numero_estrazione;";
$data = new MysqlClass ();
$data->connetti ();
...
if (@mysqli_num_rows ( $estrazione ) > 0) {
	if ($solo_tab == 1) {
		...
		while ( $rows = $data->estrai ( $estrazione ) ) {
			...
			if ($rowcont == 1) {
				echo "<li role=\"presentation\" class=\"active\"><a href=\"#estwflg$rowcont\" aria-controls=\"estwflg$rowcont\" role=\"tab\" data-toggle=\"tab\">$newDate</a></li>";
			} else {
				echo "<li role=\"presentation\"><a href=\"#estwflg$rowcont\" aria-controls=\"estwflg$rowcont\" role=\"tab\" data-toggle=\"tab\">$newDate</a></li>";
			}
			$rowcont = $rowcont + 1;
		}
...
if ($solo_estrazioni == 1) {
	...
	while ( $rows = $data->estrai ( $estrazione ) ) {
		...
		echo '<div role="tabpanel" class="tab-pane' . $active . '" id="estwflg' . $conta . '">';
		echo '<div class="row">';
		echo '<div class="col-sm-12 text-center"><h2 class="titolo row">Estrazioni Win For Life Grattacielo <br /> ' . $newDate . '</h2></div>';
		// --------------------------------------------- contenuto tab ---------------------------------------------
		$g_sql = "SELECT num, estrazione, num_one, ora FROM es_winforlife_grattacielo where esdata = '$es_data' order by esdata desc, num desc limit $num_estrazioni;";
		...
		while ( $erow = $data2->estrai ( $estrazioni10 ) ) {
			...
			$one = $erow->num_one;
			echo '<tr>';
			echo "<td colspan=\"5\">n. $num - ore $ora</td>";
			echo '</tr>';
			echo '<tr>';
			foreach ( $est as $key ) {
				echo "<td>";
			...
			echo '</tr>';
			echo '<tr>';
			echo "<td colspan=\"5\" class=\"oro nowrap\"><b>Numerone: " . check_numero($one) . "</b></td>";
			echo '</tr>';
